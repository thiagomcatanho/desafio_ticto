...
@section('content')
    <div class="bg-white rounded-lg shadow p-4">
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-2">
            ...
            <a href="{{ route('users.create_employee') }}" class="btn-form-submit">Adicionar Funcionario</a>
        </div>
                    ...
                    @foreach ($employees as $employee)
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 space-x-2">
                                <a href="{{ route('admin.edit_employee', $employee->id) }}"
                                    class="text-blue-500 hover:underline">Editar</a>
                                <button class="text-red-500 hover:underline"
                                    onclick="openDeleteModal('{{ route('admin.destroy_employee', $employee->id) }}', '{{ $employee->name }}')">
                                    Excluir
                                </button>
                            </td>
                        </tr>
